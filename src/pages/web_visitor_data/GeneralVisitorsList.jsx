...
const GeneralVisitorsList = () => {
  ...
  const filteredVisitors = generalVisitors.filter(v => {
  ...
  const totalStats = generalVisitors.length;
  const todaysStats = generalVisitors.filter(v => new Date(v.createdAt) >= startOfToday).length;
  const thisWeekStats = generalVisitors.filter(v => new Date(v.createdAt) >= startOfWeek).length;
  const thisMonthStats = generalVisitors.filter(v => new Date(v.createdAt) >= startOfMonth).length;
  const approvedStats = generalVisitors.filter(v => (v.status || "").toLowerCase().includes('approved')).length;
  ...
  const headerActions = (
    <div className="flex items-center gap-2">
      <button onClick={() => dispatch(fetchGeneralVisitors())} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded transition-colors" title="Refresh">
  ...
  const tableHeadersComponent = (
    <>
      ...
      <th className="px-2 py-2 font-medium">Area of Interest</th>
      <th className="px-2 py-2 font-medium">Registration For</th>
  ...
  const tableBodyContent = (
    <>
      {loading ? (
        ...
        <tr><td colSpan="10" className="text-center py-8 text-slate-500 text-[11px]">No general visitors found.</td></tr>
      ) : (
            ...
            <tr key={row._id || i} className="hover:bg-slate-50 transition-colors border-b border-slate-100 bg-white">
              ...
              <td className="px-2 py-2">
                <div className="font-bold text-[11px] cursor-pointer hover:text-emerald-600 hover:underline" style={{ color: '#093C5D', fontFamily: 'Inter, sans-serif' }}>
                  <Link to={`/webVisitorData/generalVisitorDetails/${row._id}`}>{toTitleCase(`${row.firstName || ""} ${row.lastName || ""}`)}</Link>
                </div>
              ...
              <td className="px-2 py-2">
                <div className="text-[9px] font-semibold text-slate-700 whitespace-pre-wrap line-clamp-2 max-w-[150px]">
                  {row.areaOfInterest?.length ? row.areaOfInterest.map(i => toTitleCase(i)).join(', ') : "-"}
                </div>
              </td>
              <td className="px-2 py-2">
                <div className="text-[9px] font-semibold text-slate-700">{toTitleCase(row.registrationFor)}</div>
              </td>
              <td className="px-2 py-2 text-center">
                {row.qrCode ? (
  ...
  return (
    <>
      ...
      <BulkUploadModal 
        ...
        uploadUrl="/api/general-visitors/upload"
        templatePath="/templates/GeneralVisitorTemplate.xlsx"
        title="Bulk Upload General Visitors"
        onSuccess={() => dispatch(fetchGeneralVisitors())}
      />
    </>
  );
};
