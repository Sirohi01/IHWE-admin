...
const FabricationPartnerManage = () => {
    ...
    const [formData, setFormData] = useState({
        hero: {
            title: "", subtitle: "", slogan: "", badgeText: "", partnerTitle: "", fabricationPartnerLabel: "", fabricationPartnerDesc: "", whyPartnerTitle: "", whyPartnerItems: []
        },
        stats: [],
        benefits: {
    ...
    const fetchData = async () => {
        try {
            const response = await api.get(`/api/fabrication-partner?t=${new Date().getTime()}`);
            if (response.data.success && response.data.data) {
                const data = JSON.parse(JSON.stringify(response.data.data));
                setFormData({
                    hero: data.hero || { title: "", subtitle: "", slogan: "", badgeText: "", partnerTitle: "", fabricationPartnerLabel: "", fabricationPartnerDesc: "", whyPartnerTitle: "", whyPartnerItems: [] },
                    stats: data.stats || [],
                    benefits: {
                        companyCard: {
            ...
            Swal.fire("Error", "Failed to load fabrication partner data", "error");
        }
    };
    ...
    const uploadImage = async (file, type) => {
        ...
        try {
            await api.post("/api/fabrication-partner/upload", imageFormData, {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            await api.put("/api/fabrication-partner", formData);
            if (heroImage) await uploadImage(heroImage, 'hero');
            if (footerImage) await uploadImage(footerImage, 'footer');
            Swal.fire("Success", "Fabrication Partner content updated successfully", "success");
            fetchData();
        } catch (error) {
    ...
    return (
        <div className="bg-gray-50 min-h-screen p-6 mt-8">
            <div className="flex justify-between items-center mb-6">
                <PageHeader title="FABRICATION PARTNER EDITOR" description="Manage all sections of the Stall Designer & Fabrication Partner page using tabs." />
                <button
                    onClick={fetchData}
                    ...
                    {activeTab === 'hero' && (
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className={labelClass}>Fabrication Partner Label</label>
                                    <input type="text" value={formData.hero.fabricationPartnerLabel} onChange={e => handleTextChange('hero', 'fabricationPartnerLabel', e.target.value)} className={inputClass} />
                                </div>
                                <div>
                                    <label className={labelClass}>Fabrication Partner Description</label>
                                    <textarea value={formData.hero.fabricationPartnerDesc} onChange={e => handleTextChange('hero', 'fabricationPartnerDesc', e.target.value)} className={inputClass} rows={3} />
                                </div>
                                <div>
                                ...
                                <div>
                                    <label className={labelClass}>Official Fabrication Partner Badge Text (Use Enter key for lines)</label>
                    ...
                    {activeTab === 'stats' && (
                        <div className="space-y-6">
                            <div className="pb-2 border-b flex items-center justify-between">
                                ...
                                <button type="button" onClick={() => addArrayItem('stats', null, { value: '', label: '', icon: 'Star', color: '#04777a' })} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded text-xs flex items-center gap-1.5 font-bold transition-colors shadow-sm"><Plus className="w-4 h-4" /> Add Stat Card</button>
                            </div>
                                    ...
                                    <div key={idx} className="p-4 border border-gray-200 rounded-lg bg-gray-50 relative">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div>
                                                ...
                                                <input type="color" value={stat.color || "#04777a"} onChange={e => {
                                                    const ns = [...formData.stats];
                    ...
                    {activeTab === 'packages' && (
                        <div className="space-y-6">
                            <div className="pb-2 border-b flex items-center justify-between">
                                ...
                                <button
                                    type="button"
                                    onClick={() => addArrayItem('packages', 'items', { name: '', price: '', icon: 'Star', color: '#00767a', titleColor: '#ffffff' })}
...
export default FabricationPartnerManage;
