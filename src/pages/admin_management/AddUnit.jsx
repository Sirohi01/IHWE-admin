...
import {
    ...
    deleteUnit,
    updateUnitStatus,
} from "../../features/add_by_admin/unit/UnitSlice";

export default function AddUnit() {
    const dispatch = useDispatch();
    const { units = [], loading = false } = useSelector((state) => state.unit || {});

    const [form, setForm] = useState({ unit: "", status: "" });
    ...
    const handleSubmit = async () => {
        if (!form.unit || !form.status) return alert("Please fill all fields.");
        ...
        if (isEditing) {
            try {
                await dispatch(updateUnit({ id: editingId, updatedData: payload })).unwrap();
                resetForm();
            } catch (error) {
            ...
            try {
                await dispatch(addUnit(payload)).unwrap();
                resetForm();
            } catch (error) {
    ...
    const handleEdit = (item) => {
        ...
        setForm({ unit: item.unit, status: item.status });
    };
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl font-bold text-[#23471d] mb-1">Manage Units</h1>
            <p className="text-gray-500 mb-6 text-sm">Manage units and their status</p>

            {/* Form Card */}
            <div className="bg-white border border-gray-200 rounded-lg p-5 mb-6 shadow-sm">
                <p className="text-sm font-medium text-gray-600 mb-4">
                    {isEditing ? `Editing: ${form.unit}` : "Add new unit"}
                </p>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                <div className="bg-[#23471d] px-5 py-3">
                    <h2 className="text-white font-medium">Unit list</h2>
                </div>
                <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500">
                        <tr>
                            <th className="text-left px-4 py-3 font-medium">S.No</th>
                            <th className="text-left px-4 py-3 font-medium">Unit Name</th>
