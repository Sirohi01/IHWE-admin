...
const AccountNavigation = ({ id, accountName, pageName, hasExhibitorRegistration = true }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const eventQuery = location.search || '';

  if (!id || id === 'all') return null;

  const navItems = [
    { label: 'Proforma Invoices', path: `/performa-invoice-list/${id}`, icon: <FileText size={12} />, availableBeforeBooking: true },
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2 mb-2 bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm w-full">
      ...
      <div className="flex items-center gap-1 overflow-x-auto hide-scrollbar w-full lg:w-auto">
        <button
          onClick={() => navigate(`/dashboard/account/${id}${eventQuery}`)}
        ...
        {navItems.map((item, idx) => {
          ...
          const isDisabled = !hasExhibitorRegistration && !item.availableBeforeBooking;
          return (
            <button
              key={idx}
              disabled={isDisabled}
              title={isDisabled ? 'Book a Stand for this exhibition to unlock this option' : item.label}
              onClick={() => navigate(`${item.path}${eventQuery}`)}
