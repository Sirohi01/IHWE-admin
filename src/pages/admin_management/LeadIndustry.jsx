...
export default function LeadIndustry() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await api.get("/api/lead-industry");
      ...
      console.error("Error fetching Lead Industry / Sector:", err);
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    if (!form.name.trim()) {
      return Swal.fire("Warning", "Please enter the Industry / Sector Name", "warning");
    }
    setIsLoading(true);
    const payload = {
      name: form.name.trim(),
    ...
    try {
      if (isEditing) {
        await api.put(`/api/lead-industry/${editingId}`, {
          ...payload,
        ...
        await api.post("/api/lead-industry", payload);
        Swal.fire({
          icon: "success",
          title: "Added!",
          timer: 1500,
  ...
  const deleteItem = async (id) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "This industry / sector option will be removed from Lead forms.",
      icon: "warning",
    ...
    setIsLoading(true);
    try {
      await api.delete(`/api/lead-industry/${id}`);
      Swal.fire("Deleted!", "Industry option removed.", "success");
      fetchData();
    } catch (err) {
  ...
  const toggleStatus = async (item) => {
    ...
    try {
      await api.put(`/api/lead-industry/${item._id}`, {
        status: newStatus,
        updated_by: getAdminId(),
      });
      fetchData();
  ...
  return (
    <div className="bg-white shadow-md p-6 min-h-screen">
      <PageHeader
        title="LEAD: INDUSTRY / SECTOR"
        description="Manage the Industry / Sector dropdown options shown in the Add New Lead registration form"
      />
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white border-2 border-gray-200 p-6 shadow-sm">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-[#d26019]">
              ...
              {isEditing ? "Edit Industry / Sector" : "Add New Industry / Sector"}
            </h2>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">
                  Industry / Sector Name <span className="text-red-500">*</span>
                </label>
                <input
                  required
                  type="text"
              ...
              <div className="flex gap-2 pt-2">
                ...
                >
                  {isEditing ? (
                    <>
                      <Edit className="w-4 h-4" /> Update Industry
                    </>
                  ) : (
                    <>
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 font-medium italic">
                <li>
                  ...
                  <strong>Industry / Sector</strong> dropdown in the{" "}
            ...
            <div className="px-6 py-4 border-b bg-[#23471d] flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <List className="w-5 h-5 text-[#d26019]" /> Industry / Sector Options
                </h2>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 text-gray-600 text-[10px] uppercase font-black tracking-widest border-b">
                    ...
                    <th className="px-5 py-4">Industry / Sector Name</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {isLoading ? (
                    ...
                    <tr>
                      ...
                      >
                        No industry options found. Add your first option above.
                      </td>
                    </tr>
                  ) : (
