...
const InvoicePreviewTemplate = ({ form, items, matchedInvoice }) => {
    ...
    return (
        <div className="bg-white border border-slate-300 p-4 text-[11px] font-sans text-black text-left" style={{ fontFamily: 'Calibri, Arial, sans-serif', maxWidth: '1000px', margin: '0 auto' }}>
            ...
            <div className="invoice-title-bar" style={{ textAlign: 'center', marginBottom: 4, paddingTop: 2, paddingBottom: 2 }}>
                <div style={{ fontWeight: 400, fontSize: 18, color: '#0d1f3c', marginBottom: 0 }}>TAX INVOICE</div>
            </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                <thead>
                    <tr>
                        ...
                        <th style={{ background: '#0d1f3c', color: '#fff', border: '1px solid #0d1f3c', padding: '3px 2px', width: '33%', textAlign: 'center', fontSize: 10, fontWeight: 'bold' }}> Invoice Details</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td style={{ border: '1px solid #ccc', padding: '6px 8px', verticalAlign: 'top', fontSize: 11 }}>
                            ...
                            <div style={{ marginTop: 4, textTransform: 'capitalize', whiteSpace: 'pre-wrap' }}>{billingAddress}</div>
                            <div style={{ marginTop: 4 }}>Contact Person: {[c1.title, c1.firstName, c1.surname].filter(Boolean).join(' ') || '—'}</div>
                            <div>Email: {c1.email || company?.email || '—'}</div>
                        ...
                        <td style={{ border: '1px solid #ccc', padding: '6px 8px', verticalAlign: 'top', fontSize: 11 }}>
                            <div style={{ fontWeight: 700, textTransform: 'uppercase' }}>{clientName || company?.companyName || '—'}</div>
                            <div style={{ marginTop: 4, whiteSpace: 'pre-wrap' }}>{shippingAddress || billingAddress}</div>
                            {gstin && <div style={{ marginTop: 4 }}>GSTIN.: {gstin}</div>}
                        </td>
                        <td style={{ border: '1px solid #ccc', padding: '6px 8px', verticalAlign: 'top', fontSize: 11 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                ...
                <tbody>
                    ...
                    {Array.from({ length: Math.max(0, 3 - (activeItems?.length || 0)) }).map((_, i) => (
            ...
            <div style={{ fontSize: 10, marginBottom: 8, padding: '6px 8px', border: '1px solid #ccc', background: '#fafafa', paddingBottom: 2 }}>
                ...
                <div style={{ whiteSpace: 'pre-wrap' }}>{termsCondition || '1. Payment must be made in favor of Namo Gange Wellness Pvt. Ltd.\n2. Full payment is due within the stipulated invoice period.'}</div>
            </div>

            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                <thead>
                    <tr style={{ background: '#0d1f3c', color: '#fff' }}>
                        <th style={{ border: '1px solid #0d1f3c', padding: '3px 2px', width: '33%', background: '#0d1f3c', color: '#fff', fontWeight: 'bold', fontSize: 10 }}>Bank Details</th>
