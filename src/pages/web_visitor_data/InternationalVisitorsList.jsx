...
const InternationalVisitorsList = () => {
  ...
  const handleBulkResend = async () => {
    ...
    if (window.confirm(`Are you sure you want to resend WhatsApp passes to ${selectedRows.length} visitor(s)?`)) {
      try {
        ...
        const res = await fetch(`${SERVER_URL}/api/international-visitors/bulk-resend`, {
          method: 'POST',
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "International Visitors");
    XLSX.writeFile(workbook, "InternationalVisitors.xlsx");
  };
  ...
  const filteredVisitors = internationalVisitors.filter(v => {
  ...
  const totalStats = internationalVisitors.length;
  const todaysStats = internationalVisitors.filter(v => new Date(v.createdAt) >= startOfToday).length;
  const thisWeekStats = internationalVisitors.filter(v => new Date(v.createdAt) >= startOfWeek).length;
  const thisMonthStats = internationalVisitors.filter(v => new Date(v.createdAt) >= startOfMonth).length;
  const approvedStats = internationalVisitors.filter(v => (v.status || "").toLowerCase().includes('approved')).length;
  ...
  const filterBar = (
    <div className="flex flex-col sm:flex-row gap-3 items-center justify-between w-full">
      <div className="relative w-full sm:w-64 shrink-0">
        ...
        <input
          type="text"
          placeholder="Search International Visitors..."
  ...
  const tableBodyContent = (
    <>
      {loading ? (
        ...
        <tr><td colSpan="11" className="text-center py-8 text-slate-500 text-[11px]">No International Visitors found.</td></tr>
      ) : (
            ...
            <tr key={row._id || i} className="hover:bg-slate-50 transition-colors border-b border-slate-100 bg-white">
              ...
              <td className="px-2 py-2">
                <div className="font-bold text-[11px] cursor-pointer hover:text-emerald-600 hover:underline" style={{ color: '#093C5D', fontFamily: 'Inter, sans-serif' }}>
                  <Link to={`/webVisitorData/InternationalVisitorDetails/${row._id}`}>{toTitleCase(`${row.firstName || ""} ${row.lastName || ""}`)}</Link>
                </div>
