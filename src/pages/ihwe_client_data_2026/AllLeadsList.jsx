...
const AllLeadsList = () => {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      dispatch(fetchCompanies({
        ...
        limit,
        search: searchTerm,
        // No status filter — fetch ALL leads
        source: filterSource,
  ...
  // Client-side status filter (since we fetch all)
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium">
            Combined view of all leads — New, Follow-Up, Hot, Lost and more
          </p>
        </div>
      ...
      <div className="flex-grow flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden min-h-0">
        ...
        <div className="p-2 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 bg-[#0A2947] rounded-t-xl">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setPage(1); }} className="py-1.5 px-2 border rounded text-[10px] font-medium outline-none cursor-pointer bg-purple-600 border-purple-500 text-white">
