...
import { Search, Trash2, Mail, Building2, User, Phone, MessageSquare, Calendar } from 'lucide-react';
import api from "../lib/api";
import Swal from 'sweetalert2';
import Table from '../components/table/Table';
import Pagination from "../components/Pagination";

const EPromotionRegisters = () => {
    ...
    const fetchEnquiries = async () => {
        try {
            setIsLoading(true);
            const response = await api.get('/api/e-promotion/enquiries');
    ...
    const handleDelete = async (enquiry) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await api.delete(`/api/e-promotion/enquiries/${enquiry._id}`);
                if (response.data.success) {
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex items-center gap-2">
                    ...
                    <div className="text-gray-900 font-medium">{row.company}</div>
                </div>
            )
        },
        {
            ...
            render: (row) => (
                <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-gray-600 font-normal italic">{row.mobile}</span>
                    </div>
                </div>
            )
        },
        ...
        <div className="bg-white shadow-md mt-6 p-6">
            <div className="w-full">
                <div className="mb-6">
                    ...
                    <p className="text-gray-600 mt-2 text-lg">List of all e-promotion enquiries submitted from the website</p>
                </div>
...
export default EPromotionRegisters;
