                ...
                <div className="flex flex-wrap gap-2">
                  {STATUSES.map(s => (
                    <button
                      key={s}
                      onClick={() => setStatus(s)}
                      className={`px-3 py-1.5 rounded-full text-[11px] font-black border transition-all ${status === s ? STATUS_COLORS[s] + " ring-2 ring-offset-1 ring-current" : "bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100"
                        }`}
                    >
                  ...
                  {paginated.map((n, idx) => (
                    <tr key={n._id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black whitespace-nowrap ${n.applicantType === 'Individual'
