...
// ✅ Helper — user info sessionStorage se lo (token hata diya)
const getUserInfo = () => {
  const userStr = sessionStorage.getItem("user");
  const user = userStr ? JSON.parse(userStr) : {};
  return {
...
export const addCompany = createAsyncThunk(
  ...
  async (companyData, { dispatch, rejectWithValue }) => {
    ...
    try {
      const res = await api.post("/api/companies", companyData); // ✅ api use
      const created = res.data.data || res.data;
...
export const updateCompany = createAsyncThunk(
  ...
  async ({ id, data }, { dispatch, rejectWithValue }) => {
    ...
    try {
      ...
      const res = await api.put(`/api/companies/${id}`, dataWithUser); // ✅ api use
      const updated = res.data.data || res.data;
...
export const deleteCompany = createAsyncThunk(
  ...
  async (id, { dispatch, getState, rejectWithValue }) => {
    ...
    try {
      ...
      await api.delete(`/api/companies/${id}`); // ✅ api use
