...
const TravelPartnerManage = () => {
    ...
    const [formData, setFormData] = useState({
        hero: {
            title: "", subtitle: "", slogan: "", badgeText: "", partnerTitle: "", travelPartnerLabel: "", travelPartnerDesc: "", whyPartnerTitle: "", whyPartnerItems: []
        },
        stats: [],
        benefits: {
    ...
    const fetchData = async () => {
        try {
            const response = await api.get(`/api/travel-partner?t=${new Date().getTime()}`);
            if (response.data.success && response.data.data) {
                const data = JSON.parse(JSON.stringify(response.data.data));
                setFormData({
                    hero: data.hero || { title: "", subtitle: "", slogan: "", badgeText: "", partnerTitle: "", travelPartnerLabel: "", travelPartnerDesc: "", whyPartnerTitle: "", whyPartnerItems: [] },
                    stats: data.stats || [],
                    benefits: {
                        companyCard: {
            ...
            Swal.fire("Error", "Failed to load travel partner data", "error");
        }
    };
    ...
    const uploadImage = async (file, type) => {
        ...
        try {
            await api.post("/api/travel-partner/upload", imageFormData, {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            await api.put("/api/travel-partner", formData);
            if (heroImage) await uploadImage(heroImage, 'hero');
            if (footerImage) await uploadImage(footerImage, 'footer');
            Swal.fire("Success", "Travel Partner content updated successfully", "success");
            fetchData();
        } catch (error) {
    ...
    return (
        <div className="bg-gray-50 min-h-screen p-6 mt-8">
            <div className="flex justify-between items-center mb-6">
                <PageHeader title="TRAVEL PARTNER EDITOR" description="Manage all sections of the Travel Partner page using tabs." />
                <button
                    onClick={fetchData}
                    ...
                    {activeTab === 'hero' && (
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className={labelClass}>Travel Partner Label</label>
                                    <input type="text" value={formData.hero.travelPartnerLabel} onChange={e => handleTextChange('hero', 'travelPartnerLabel', e.target.value)} className={inputClass} />
                                </div>
                                <div>
                                    <label className={labelClass}>Travel Partner Description</label>
                                    <textarea value={formData.hero.travelPartnerDesc} onChange={e => handleTextChange('hero', 'travelPartnerDesc', e.target.value)} className={inputClass} rows={3} />
                                </div>
                                <div>
                                ...
                                <div>
                                    <label className={labelClass}>Official Travel Partner Badge Text (Use Enter key for lines)</label>
                                    ...
                                    {formData.hero.whyPartnerItems.map((item, idx) => (
                                        <div key={idx} className="flex gap-3 bg-white p-3 rounded border border-gray-200 items-center">
                                            <div className="flex-1">
                                                <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Item Text</label>
                                                <input type="text" placeholder="Item Text" value={item.text} onChange={e => handleArrayChange('hero', 'whyPartnerItems', idx, 'text', e.target.value)} className={`${inputClass}`} />
                                            </div>
                                            <div className="w-40">
                                                <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Lucide Icon</label>
                                                <input type="text" placeholder="Icon Name" value={item.icon} onChange={e => handleArrayChange('hero', 'whyPartnerItems', idx, 'icon', e.target.value)} className={`${inputClass}`} />
                                            </div>
                                            <button type="button" onClick={() => removeArrayItem('hero', 'whyPartnerItems', idx)} className="p-2 mt-4 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4.5 h-4.5" /></button>
                                        </div>
                                    ))}
                                </div>
                            </div>
...
export default TravelPartnerManage;
