...
//   const qty = parseAmount(item.qty || item.quantity || 1) || 1;
//   const rate = parseAmount(item.rate || item.unit_rate || item.price);
//   const computedAmount = rate * getAreaMultiplier(item) * qty;
//   const explicitAmount = parseAmount(
//     item.amount ??
//     item.itemValue ??
//     item.item_value ??
//     item.totalBeforeTax ??
...
//   const discountPercent = parseAmount(item.discountPct ?? item.disc ?? item.discount_percent);
//   const discountAmount = parseAmount(item.discountAmount ?? item.discount_amount);
...
//         const gstRate = parseAmount(item.gstRate ?? item.gst_per ?? item.gstPct);
//         const gstAmount = parseAmount(item.gstAmount ?? item.gst_amount) || (gstRate ? (taxable * gstRate) / 100 : 0);
//         return {
//           ...item,
//           amount,
//           taxable,
...
//     const amount = getLineAmount(item);
//     const rateAmount = amount || (Number(item.rate || 0) * getAreaMultiplier(item) * (qty || 1));
//     const discount = Number(item.discountAmount ?? item.discount_amount ?? 0) * ratio;
...
//     return 0;
//   };
//   const discountPercentValue = (item) => {
//     const directPercent = Number(item.disc ?? item.discountPct ?? item.discount_percent);
//     if (Number.isFinite(directPercent) && directPercent) return directPercent;

//     const discountAmount = Number(item.discountAmount ?? item.discount ?? 0);
//     const amount = Number(item.amount || 0);
//     if (Number.isFinite(discountAmount) && discountAmount && Number.isFinite(amount) && amount) {
...
//             <div style={{ fontWeight: 400, fontSize: 18 }}>DELIVERY CHALLAN</div>
//             <div style={{ justifySelf: "end", fontWeight: 700, fontSize: 11 }}>{copyLabel}</div>
//           </div>

//           <table className="challan-summary-table" style={{ width: "100%", borderCollapse: "collapse", marginBottom: 8, tableLayout: "fixed" }}>
//             <thead>
//               <tr>
//                 <th className="challan-client-column" style={{ ...topTh, width: "34%" }}>Buyer's Name &amp; Address</th>
//                 <th className="challan-shipment-column" style={{ ...topTh, width: "34%" }}>Shipment Details</th>
//                 <th className="challan-details-column" style={{ ...topTh, width: "32%" }}>Delivery Challan Details</th>
//               </tr>
//             </thead>
//             <tbody>
//               <tr>
...
//                           <td style={{ ...detailValueCell, textAlign: "right", whiteSpace: "normal", overflowWrap: "anywhere", wordBreak: "break-word" }}>{value}</td>
//                         </tr>
//                       ))}
//                     </tbody>
//                   </table>
...
//           <table className="challan-bank-table" style={{ width: "100%", borderCollapse: "collapse", marginBottom: 8 }}>
//             <thead>
//               <tr>
...
//           .challan-client-column { width: 34% !important; }
//           .challan-shipment-column { width: 34% !important; }
//           .challan-details-column { width: 32% !important; }
