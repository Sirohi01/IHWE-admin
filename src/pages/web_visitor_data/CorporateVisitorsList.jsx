...
const CorporateVisitorsList = () => {
  ...
  const totalStats = corporateVisitors.length;
  const todaysStats = corporateVisitors.filter(v => new Date(v.createdAt) >= startOfToday).length;
  const thisWeekStats = corporateVisitors.filter(v => new Date(v.createdAt) >= startOfWeek).length;
  const thisMonthStats = corporateVisitors.filter(v => new Date(v.createdAt) >= startOfMonth).length;
  const approvedStats = corporateVisitors.filter(v => (v.status || "").toLowerCase().includes('approved')).length;
  ...
  return (
    <>
      ...
      <BulkUploadModal 
        ...
        uploadUrl="/api/corporate-visitors/upload"
        templatePath="/templates/CorporateVisitorTemplate.xlsx"
        title="Bulk Upload Corporate Visitors"
        onSuccess={() => dispatch(fetchCorporateVisitors())}
      />
    </>
  );
};
