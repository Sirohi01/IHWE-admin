...
export default function AccessoryCategory() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await api.get("/api/accessory-categories");
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    if (!form.name.trim()) {
      return Swal.fire("Warning", "Please enter the Category Name", "warning");
    }
    setIsLoading(true);
    const payload = {
      name: form.name.trim(),
    ...
    try {
      if (isEditing) {
        await api.put(`/api/accessory-categories/${editingId}`, {
          ...payload,
        ...
        await api.post("/api/accessory-categories", payload);
        Swal.fire({
          icon: "success",
          title: "Added!",
          timer: 1500,
  ...
  const deleteItem = async (id) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "This category option will be removed from Stall Accessory forms.",
      icon: "warning",
    ...
    setIsLoading(true);
    try {
      await api.delete(`/api/accessory-categories/${id}`);
      Swal.fire("Deleted!", "Category option removed.", "success");
      fetchData();
    } catch (err) {
  ...
  const toggleStatus = async (item) => {
    ...
    try {
      await api.put(`/api/accessory-categories/${item._id}`, {
        status: newStatus,
        updated_by: getAdminId(),
      });
      fetchData();
  ...
  return (
    <div className="bg-white shadow-md p-6 min-h-screen">
      <PageHeader
        title="ACCESSORY: ITEM CATEGORY"
        description="Manage the Item Category dropdown options shown in the Create New Accessory modal form"
      />
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 font-medium italic">
                <li>
                  ...
                  <strong>Item Category</strong> dropdown in the{" "}
                  <strong>Create New Accessory</strong> modal form.
                </li>
            ...
            <div className="px-6 py-4 border-b bg-[#23471d] flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <List className="w-5 h-5 text-[#d26019]" /> Category Options
                </h2>
                ...
                <tbody className="divide-y divide-gray-100">
                  {isLoading ? (
                    ...
                    <tr>
                      ...
                      >
                        No category options found. Add your first option above.
                      </td>
                    </tr>
                  ) : (
