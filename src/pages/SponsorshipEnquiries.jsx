...
import { Search, Trash2, Mail, Phone, User, Calendar, Tag, Building2 } from 'lucide-react';
import api from "../lib/api";
import Swal from 'sweetalert2';
import Table from '../components/table/Table';
import Pagination from "../components/Pagination";

const SponsorshipEnquiries = () => {
    ...
    const fetchEnquiries = async () => {
        try {
            setIsLoading(true);
            const response = await api.get('/api/sponsorship-enquiry');
    ...
    const handleDelete = async (enquiry) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await api.delete(`/api/sponsorship-enquiry/${enquiry._id}`);
                if (response.data.success) {
    ...
    const filteredEnquiries = enquiries.filter(enq => 
        ...
        enq.category?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex items-center gap-2">
                    ...
                    <div className="text-gray-900 font-bold uppercase text-[10px] tracking-tight">{row.category}</div>
                </div>
            )
        },
        {
            ...
            render: (row) => (
                <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-red-600 font-semibold italic">{row.phone}</span>
                    </div>
                </div>
            )
        },
        {
            ...
            label: "MESSAGE",
            render: (row) => (
                <div className="max-w-[300px] text-xs font-bold line-clamp-2 italic text-black">
    ...
    return (
        <div className="bg-white shadow-md mt-6 p-6">
            <div className="w-full">
                <div className="mb-6">
                    <h1 className="text-3xl font-bold text-[#23471d]">SPONSORSHIP ENQUIRIES</h1>
                    <p className="text-gray-600 mt-2 text-lg">List of all inquiries submitted from the Sponsorship section</p>
                </div>
                    ...
                    <div className="px-6 py-4 border-b bg-[#23471d]">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="relative w-72">
                                ...
                                <input
                                    type="text"
                                    placeholder="Search sponsorship enquiries..."
                                    value={searchTerm}
